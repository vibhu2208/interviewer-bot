...
export class Athena {
  ...
  static async query<T>(query: string): Promise<T[]> {
    ...
    do {
      ...
      console.log(`Fetched ${result.ResultSet?.Rows?.length} rows for ${queryExecutionId}`);
  ...
  static async startQueryExecution(query: string): Promise<string | null> {
    const athenaQuery = await client.send(
      new StartQueryExecutionCommand({
        ResultConfiguration: {
          OutputLocation: `s3://${Config.getDataBucketName()}/athena-tmp`,
        },
        QueryString: query,
    ...
    console.log(`ATHENA_QueryExecutionId: ${queryExecutionId}`);
