...
export class GoogleColab {
  ...
  static async default(): Promise<DriveClient> {
    ...
    return GoogleColab.getApiClient(credentials);
  }

  /**
   * Checks if a Google Drive link is a Colab notebook
   * */
  static canBeColabNotebook(driveLink: string): boolean {
    return (
      driveLink.includes('/notebook/d/') ||
      driveLink.includes('https://colab.research.google.com/drive/') ||
      driveLink.includes('https://drive.google.com/file/d')
    );
  }
  ...
  static async exportAsJsonString(driveLink: string): Promise<string> {
    ...
    try {
      ...
      if (e?.message === 'Requested entity was not found.') {
        throw new NonRetryableError(`Google Drive Document '${documentId}' does not exist`);
      }
      if (e?.message === 'The caller does not have permission') {
        throw new NonRetryableError(`Google Drive Document '${documentId}' does not share access`);
      }
...
export function getFileIdFromUrl(link: string): string {
  ...
  try {
    ...
    throw new NonRetryableError(`Invalid URL format: '${link}'`);
  }
  ...
  if (documentId == null || documentId.trim().length === 0) {
    throw new NonRetryableError(`Cannot extract documentId from the link: '${link}'`);
  }
